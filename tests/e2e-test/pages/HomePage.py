...
class BIABPage(BasePage):
    ...
    ANALYSIS_RECOMMENDATION_AGENT = (
        "//span[normalize-space()='Analysis Recommendation Agent']"
    )
    ...
    def select_retail_customer_success_team(self):
        ...
        logger.info(
            "Validating 'Retail Customer Success Team' is selected and visible..."
        )
    ...
    def validate_retail_agents_visible(self):
        ...
        expect(self.page.locator(self.ANALYSIS_RECOMMENDATION_AGENT)).to_be_visible(
            timeout=10000
        )
    ...
    def validate_rfp_agents_visible(self):
        ...
        expect(self.page.locator(self.RFP_COMPLIANCE_AGENT)).to_be_visible(
            timeout=10000
        )
    ...
    def approve_retail_task_plan(self):
        ...
        try:
            if (
                clarification_input.is_visible(timeout=5000)
                and clarification_input.is_enabled()
            ):
                logger.error(
                    "⚠ Clarification input is enabled - Task plan approval requires clarification"
                )
    ...
    def approve_product_marketing_task_plan(self):
        ...
        try:
            if (
                ...
                logger.info(
                    "⚠ Clarification input is enabled - Providing product marketing details"
                )

                # Fill in product marketing clarification details
                pm_clarification = (
                    "company name : Contoso, Contact details: 1234567890, "
                ...
                logger.info(
                    "Waiting for 'Processing your plan' message after clarification..."
                )
                expect(self.page.locator(self.PROCESSING_PLAN)).to_be_visible(
                    timeout=15000
                )
                logger.info(
                    "✓ 'Processing your plan' message is visible after clarification"
                )
                logger.info("Waiting for plan processing to complete...")
                self.page.locator(self.PROCESSING_PLAN).wait_for(
                    state="hidden", timeout=200000
                )
                logger.info("✓ Plan processing completed")
            else:
                logger.info("✓ No clarification required - task completed successfully")
    ...
    def approve_rfp_task_plan(self):
        ...
        try:
            if (
                ...
                logger.error(
                    "⚠ Clarification input is enabled - RFP Task plan approval requires clarification"
                )
                raise ValueError("INPUT_CLARIFICATION is enabled - retry required")
            logger.info("✓ No clarification required - task completed successfully")
        except ValueError:
    ...
    def approve_contract_compliance_task_plan(self):
        ...
        try:
            if (
                ...
                logger.error(
                    "⚠ Clarification input is enabled - Contract Compliance Task plan approval requires clarification"
                )
                raise ValueError("INPUT_CLARIFICATION is enabled - retry required")
            logger.info("✓ No clarification required - task completed successfully")
        except ValueError:
        ...
        logger.info(
            "Contract Compliance task plan approval and processing completed successfully!"
        )
    ...
    def validate_retail_customer_response(self):
        ...
        expect(
            self.page.locator(self.RETAIL_CUSTOMER_RESPONSE_VALIDATION)
        ).to_be_visible(timeout=10000)
        logger.info("✓ Retail customer response is visible")
        expect(self.page.locator(self.RETAIL_COMPLETED_TASK).first).to_be_visible(
            timeout=6000
        )
        ...
        try:
            expect(self.page.locator(self.ORDER_DATA).first).to_be_visible(
                timeout=10000
            )
        ...
        try:
            expect(self.page.locator(self.CUSTOMER_DATA).first).to_be_visible(
                timeout=10000
            )
        ...
        try:
            expect(self.page.locator(self.ANALYSIS_RECOMMENDATION).first).to_be_visible(
                timeout=10000
            )
            logger.info("✓ Analysis Recommendation is visible")
        except (AssertionError, TimeoutError) as e:
            logger.warning(
                f"⚠ Analysis Recommendation Agent is NOT Utilized in response: {e}"
            )
    ...
    def validate_product_marketing_response(self):
        ...
        expect(self.page.locator(self.PM_COMPLETED_TASK).first).to_be_visible(
            timeout=6000
        )
    ...
    def validate_hr_response(self):
        ...
        try:
            expect(self.page.locator(self.TECH_SUPPORT).first).to_be_visible(
                timeout=10000
            )
            logger.info("✓ Technical Support is visible")
        except (AssertionError, TimeoutError) as e:
            logger.warning(
                f"⚠ Technical Support Agent is NOT Utilized in response: {e}"
            )
    ...
    def validate_rfp_response(self):
        ...
        expect(self.page.locator(self.RFP_RESPONSE_VALIDATION)).to_be_visible(
            timeout=20000
        )
        ...
        try:
            expect(self.page.locator(self.RFP_SUMMARY).first).to_be_visible(
                timeout=10000
            )
        ...
        try:
            expect(self.page.locator(self.RFP_COMPLIANCE).first).to_be_visible(
                timeout=10000
            )
    ...
    def validate_contract_compliance_response(self):
        ...
        expect(self.page.locator(self.CC_RESPONSE_VALIDATION)).to_be_visible(
            timeout=20000
        )
        ...
        try:
            expect(self.page.locator(self.CONTRACT_SUMMARY).first).to_be_visible(
                timeout=10000
            )
        ...
        try:
            expect(self.page.locator(self.CONTRACT_RISK).first).to_be_visible(
                timeout=10000
            )
        ...
        try:
            expect(self.page.locator(self.CONTRACT_COMPLIANCE).first).to_be_visible(
                timeout=10000
            )
            logger.info("✓ Contract Compliance is visible")
        except (AssertionError, TimeoutError) as e:
            logger.warning(
                f"⚠ Contract Compliance Agent is NOT Utilized in response: {e}"
            )
    ...
    def validate_rai_error_message(self):
        ...
        expect(self.page.locator(self.UNABLE_TO_CREATE_PLAN)).to_be_visible(
            timeout=10000
        )
    ...
    def validate_rai_clarification_error_message(self):
        ...
        logger.info(
            "Validating RAI 'Failed to submit clarification' message is visible..."
        )
    ...
    def validate_home_input_visible(self):
        ...
        expect(self.page.locator(self.HOME_INPUT_TITLE_WRAPPER)).to_be_visible(
            timeout=10000
        )
