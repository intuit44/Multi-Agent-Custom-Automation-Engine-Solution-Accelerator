...
class TestCosmosDBCRUDOperations:
    ...
    async def test_add_item_success(self, client):
        ...
        client.container.create_item.assert_called_once_with(
            body={"id": "test_id", "data": "test_data"}
        )
    ...
    async def test_update_item_success(self, client):
        ...
        client.container.upsert_item.assert_called_once_with(
            body={"id": "test_id", "data": "updated_data"}
        )
    ...
    async def test_get_item_by_id_success(self, client):
        ...
        result = await client.get_item_by_id(
            "test_id", "partition_key", mock_model_class
        )

        assert result == mock_instance
        client.container.read_item.assert_called_once_with(
            item="test_id", partition_key="partition_key"
        )
    ...
    async def test_delete_item_success(self, client):
        ...
        client.container.delete_item.assert_called_once_with(
            item="test_id", partition_key="partition_key"
        )
...
class TestCosmosDBQueryOperations:
    ...
    def client(self):
        ...
        Note: container.query_items is sync and returns an async iterator,
        so we use Mock for the container and configure specific methods as needed.
        """
        client = CosmosDBClient(
            endpoint="https://test.documents.azure.com:443/",
            credential="test_credential",
        ...
        client._initialized = True
        # Use Mock (not AsyncMock) for container since query_items is sync
    ...
    async def test_query_items_with_validation_error(self, client):
        ...
        mock_model_class = Mock()
        mock_instance = Mock()
        mock_model_class.model_validate.side_effect = [
            mock_instance,
...
class TestCosmosDBPlanOperations:
    ...
    async def test_get_plan_by_plan_id_found(self, client):
        ...
        client.query_items.assert_called_once_with(
            expected_query, expected_params, Plan
        )
    ...
    async def test_get_all_plans(self, client):
        ...
        expected_query = (
            "SELECT * FROM c WHERE c.user_id=@user_id AND c.data_type=@data_type"
        )
        expected_params = [
            {"name": "@user_id", "value": "test_user"},
            {"name": "@data_type", "value": DataType.plan},
    ...
    async def test_get_all_plans_by_team_id_status(self, client):
        ...
        result = await client.get_all_plans_by_team_id_status(
            "user123", "team456", "active"
        )
...
class TestCosmosDBStepOperations:
    ...
    async def test_get_steps_by_plan(self, client):
        ...
        client.query_items.assert_called_once_with(
            expected_query, expected_params, Step
        )
...
class TestCosmosDBTeamOperations:
    ...
    async def test_get_team_found(self, client):
        ...
        expected_query = (
            "SELECT * FROM c WHERE c.team_id=@team_id AND c.data_type=@data_type"
        )
        expected_params = [
            {"name": "@team_id", "value": "test_team_id"},
        ...
        client.query_items.assert_called_once_with(
            expected_query, expected_params, TeamConfiguration
        )
    ...
    async def test_get_all_teams(self, client):
        ...
        expected_query = (
            "SELECT * FROM c WHERE c.data_type=@data_type ORDER BY c.created DESC"
        )
        expected_params = [
            {"name": "@data_type", "value": DataType.team_config},
        ]
    ...
    async def test_delete_team_success(self, client):
        ...
        client.delete_item.assert_called_once_with(
            item_id="test_id", partition_key="test_session"
        )
...
class TestCosmosDBCurrentTeamOperations:
    ...
    async def test_get_current_team_found(self, client):
        ...
        expected_query = (
            "SELECT * FROM c WHERE c.data_type=@data_type AND c.user_id=@user_id"
        )
        expected_params = [
        ...
        client.query_items.assert_called_once_with(
            expected_query, expected_params, UserCurrentTeam
        )
    ...
    async def test_delete_current_team(self, client):
        ...
        mock_docs = [
            {"id": "doc1", "session_id": "session1"},
            {"id": "doc2", "session_id": "session2"},
        ]
...
class TestCosmosDBDataManagement:
    ...
    async def test_get_data_by_type_with_unmapped_class(self, client):
        ...
        client.query_items.assert_called_once_with(
            expected_query, expected_params, BaseDataModel
        )
...
class TestCosmosDBAgentMessageOperations:
    ...
    async def test_get_agent_messages(self, client):
        ...
        client.query_items.assert_called_once_with(
            expected_query, expected_params, AgentMessageData
        )
...
class TestCosmosDBMiscellaneousOperations:
    ...
    async def test_get_team_agent(self, client):
        ...
        client.query_items.assert_called_once_with(
            expected_query, expected_params, CurrentTeamAgent
        )
