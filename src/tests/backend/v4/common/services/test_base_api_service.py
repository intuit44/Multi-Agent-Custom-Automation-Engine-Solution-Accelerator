...
class TestBaseAPIService:
    ...
    def test_url_with_complex_path(self):
        ...
        assert service._url(
            "users/123/profile") == "https://api.example.com/users/123/profile"

    @pytest.mark.asyncio
    async def test_request_method(self):
        """Test _request method with various parameters."""
        service = BaseAPIService("https://api.example.com",
                                 default_headers={"Auth": "token"})
    ...
    async def test_get_json_with_http_error(self):
        ...
        mock_response = Mock(spec=aiohttp.ClientResponse)
        mock_response.raise_for_status = Mock(
            side_effect=aiohttp.ClientError("404 Not Found"))
    ...
    async def test_post_json_with_http_error(self):
        ...
        mock_response = Mock(spec=aiohttp.ClientResponse)
        mock_response.raise_for_status = Mock(
            side_effect=aiohttp.ClientError("400 Bad Request"))
    ...
    async def test_integration_workflow(self):
        ...
        mock_get_response.json = AsyncMock(
            return_value={"users": [{"id": 1, "name": "Alice"}]})

        # Mock POST response
        mock_post_response = Mock(spec=aiohttp.ClientResponse)
        mock_post_response.raise_for_status = Mock()
        mock_post_response.json = AsyncMock(
            return_value={"id": 2, "name": "Bob", "created": True})

        mock_session.request = AsyncMock(
            side_effect=[mock_get_response, mock_post_response])
