...
class TestProxyAgentComplexScenarios:
    ...
    def test_agent_run_response_patterns(self):
        ...
        def build_agent_response(updates):
            ...
            return {"messages": response_messages, "response_id": response_id}

        # Mock updates
        mock_updates = [
            type("Update", (), {"contents": ["Hello"], "role": "assistant"})(),
            type(
                "Update", (), {"contents": ["How can I help you?"], "role": "assistant"}
            )(),
        ]
    ...
    def test_websocket_communication_patterns(self):
        ...
        async def test_websocket():
            ...
            result_timeout = await simulate_send_clarification_request(
    ...
    def test_logging_patterns(self):
        ...
        def simulate_logging_calls():
            ...
            log_messages.append(
                (
                    "INFO",
                    "ProxyAgent: Requesting clarification (thread=present, user=test_user)",
                )
            )

            # Simulate debug logging
            log_messages.append(
                ("DEBUG", "ProxyAgent: Message text: Please help me with this request")
            )
            ...
            log_messages.append(
                (
                    "ERROR",
                    "ProxyAgent: Failed to send timeout notification: Connection failed",
                )
            )
...
class TestProxyAgentDirectFunctionTesting:
        ...
        async def mock_wait_for_user_clarification_success(request_id):
            ...
            try:
                ...
                return mock_user_clarification_response(
                    request_id=request_id, answer=user_answer
                )
            finally:
                # Simulate cleanup
    ...
    async def test_notify_timeout_logic(self):
        ...
        mock_orchestration_config.cleanup_clarification.assert_called_once_with(
            "test-request-id"
        )
    ...
    async def test_invoke_stream_internal_logic(self):
        ...
        mock_user_clarification_request.return_value = (
            mock_clarification_request_instance
        )
        ...
        mock_agent_run_response_update.side_effect = [
            mock_text_update,
            mock_usage_update,
        ]
        ...
        with patch("uuid.uuid4", return_value="test-uuid"):
            updates = []
            async for update in mock_invoke_stream_internal(
                "Test message", "test-user", "ProxyAgent", 300
            ):
                updates.append(update)
    ...
    def test_initialization_logic(self):
        ...
        def mock_proxy_agent_init(
            user_id=None, name="ProxyAgent", description=None, timeout_seconds=None
        ):
            ...
            mock_instance.description = (
                description or f"Human-in-the-loop proxy agent for {name}"
            )
            mock_instance._timeout = (
                timeout_seconds or mock_orchestration_config.default_timeout
            )
    ...
    def test_error_handling_patterns(self):
        ...
        try:
            result = loop.run_until_complete(
                mock_wait_with_error_handling("test-request")
            )
            assert result is None
...
class TestCoverageExtensionScenarios:
    ...
    def test_websocket_message_types(self):
        ...
        assert (
            mock_websocket_message_type.USER_CLARIFICATION_REQUEST
    ...
    def test_mock_object_interactions(self):
        ...
        mock_response = mock_user_clarification_response(
            request_id="test-id", answer="test answer"
        )
    ...
    def test_content_creation_patterns(self):
        ...
        response_update = mock_agent_run_response_update(
            contents=[text_content], role=mock_role.ASSISTANT
        )
