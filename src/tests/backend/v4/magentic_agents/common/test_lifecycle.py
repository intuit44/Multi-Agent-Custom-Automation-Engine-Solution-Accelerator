...
class TestMCPEnabledBase:
    ...
    async def test_open_method_success(self):
        ...
        base = MCPEnabledBase(
            project_endpoint="https://test-endpoint.com", mcp=self.mock_mcp_config
        )
    ...
    async def test_close_method_with_exceptions(self):
        ...
        mock_agent_registry.unregister_agent.side_effect = Exception(
            "Unregister failed"
        )
    ...
    def test_getattr_delegation_no_agent(self):
        ...
        assert "MCPEnabledBase has no attribute 'nonexistent_method'" in str(
            exc_info.value
        )
