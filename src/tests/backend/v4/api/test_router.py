...
class TestRouterCoverage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        modules_to_remove = [
            name for name in sys.modules.keys() if "backend.v4.api.router" in name
        ]
    ...
    def test_router_import_with_mocks(self):
        ...
        self.mock_modules[
            "common.models.messages_af"
        ].TeamSelectionRequest = MockTeamSelectionRequest
        ...
        self.mock_modules["common.utils.utils_af"].rai_validate_team_config = Mock(
            return_value=True
        )
        ...
        ].DatabaseFactory.get_database = Mock(return_value=mock_db)
    ...
    def test_static_analysis(self):
        ...
        if os.path.exists(router_path):
            ...
            self.assertGreater(
                len(imports), 1, f"Should have imports. Found {len(imports)}"
            )
            print(
                f"Router file analysis: {len(functions)} functions, {len(imports)} imports"
            )
        else:
