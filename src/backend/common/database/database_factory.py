    ...
    async def get_database(
        ...
        if force_new or DatabaseFactory._instance is None:
            cosmos_db_client = CosmosDBClient(
                ...
                # Key (dev) wins over AAD: the account trusts a foreign
                # creation-time tenant — see COSMOSDB_KEY in app_config.
                credential=config.COSMOSDB_KEY or config.get_azure_credentials(),
