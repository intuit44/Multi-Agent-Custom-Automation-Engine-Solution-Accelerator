...
def check_team_exists(backend_url, team_id, user_principal_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Error checking team {team_id}: Status {response.status_code}, Response: {response.text}"
            )
            return False
...
if len(sys.argv) < 3:
    print(
        "Usage: python upload_team_config.py <backend_endpoint> <directory_path> [<user_principal_id>] [<team_id_from_arg>]"
    )
    sys.exit(1)
...
team_id_from_arg = (
    sys.argv[4] if len(sys.argv) > 4 else "00000000-0000-0000-0000-000000000001"
)
            ...
            try:
                with open(file_path, "rb") as file_data:
                    ...
                    response = requests.post(
                        upload_endpoint, files=files, headers=headers, params=params
                    )
                    if response.status_code == 200:
                        try:
                            ...
                            if resp_json.get("status") == "success":
                                print(
                                    f"Successfully uploaded team configuration: {resp_json.get('name')} (team_id: {resp_json.get('team_id')})"
                                )
                                uploaded_count += 1
                            else:
                                print(
                                    f"Upload failed for {filename}. Response: {resp_json}"
                                )
                                sys.exit(1)
                        except Exception as e:
